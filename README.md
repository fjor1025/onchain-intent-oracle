...
| `--no-logs` | No (flag) | Skip event-log fetching/decoding (`eth_getLogs`). Logs are fetched and decoded by default at every `--depth` level; this is an escape hatch for providers that can't serve `eth_getLogs` either, or to save the round trip when you only care about calldata/traces |
| `--entity-key <name>` | No | Force a specific decoded argument name (e.g. `id`, `vaultId`) to bucket transactions by, instead of the built-in name+type / struct-hash heuristics. Use when you know a singleton contract's shape and the heuristics don't already catch it |
...
`Source Resolver` resolves a contract's verified ABI (Etherscan-family explorers via `ETHERSCAN_API_KEY`) and uses it for authoritative method decoding — not just the method *name* (computed directly from the ABI's own `keccak256(signature)`), but the full **argument values** too, via `ingestion/calldata_decoder.py`: struct/tuple-typed parameters (e.g. Morpho Blue's `MarketParams`) are correctly expanded from the ABI's `components` shape rather than read as the literal string `"tuple"`, which was silently wrong before. Without a verified ABI, calldata still gets decoded positionally whenever `SignatureDecoder`'s builtin table or 4byte.directory fallback resolves a full typed signature, just with lower confidence and placeholder (`arg0`, `arg1`, ...) names. Standards detection (ERC-20/721/1155) also runs off the same resolved ABI. All of this is skipped at `--depth quick` to save the extra network round trip, and silently falls back to a 4byte.directory best-effort guess if no key is set or the contract isn't verified. There's no separate log-indexing module as a distinct pipeline stage, but event logs *are* fetched (`eth_getLogs`, via `RPCManager.get_logs()`) and decoded (`ingestion/log_decoder.py`) at every `--depth` level, including `quick` — unlike traces, `eth_getLogs` needs no `debug_*`/`trace_*` RPC support, so it's the one evidence channel available on every provider tier. Decoded logs feed the state machine and invariant miner as first-class evidence alongside state diffs; see `observed_design.json`'s `decoded_logs` and `decoded_calls` sections for per-run coverage (how many resolved and via which confidence tier, how many are still unresolved).

**State Machine Inference is per-entity, not just per-contract, when the contract calls for it.** Many high-value DeFi contracts are singletons that multiplex many independent logical entities behind one address — Morpho Blue's markets, Uniswap V3's per-position NFTs, any vault factory's per-vault state. Treating "the contract address" as one behavioral entity blends all of those together into a state machine that doesn't correspond to anything real. `analysis/entity_key.py` looks for a decoded argument (from a call or an event) whose type is `bytes32`/`uint256` and whose name reads as an entity identifier (`id`, `marketId`, `poolId`, `tokenId`, etc.), plus a struct-hash fallback for contracts that derive an id by hashing a params struct (Morpho's exact `Id.wrap(keccak256(abi.encode(marketParams)))` idiom) — the struct-hash candidate is only promoted to a real entity key when it's corroborated by an id value actually observed elsewhere in the same run, never used alone. When entities are found, `oio analyze` computes a separate state machine and invariant set per entity (ranked by tx count, capped and shown in full via `observed_design.json`'s `entities` section) *in addition to* the aggregate, contract-wide pass — the aggregate is explicitly labeled cross-entity in the markdown output once this happens, since some claims (e.g. "never accepts ETH") are genuinely contract-wide while state-transition claims are not once multiple entities are involved. Flat, non-multiplexed contracts (the common case) produce no entity-key candidates at all and behave exactly as before. Use `--entity-key <arg_name>` to force a specific argument name instead of relying on the heuristics.

**Every run gets a block-range activity preflight before the analysis is presented as trustworthy.** `analysis/sample_quality.py` checks the analyzed range for four independent degeneracy signals: `single_caller_dominant` (≥90% of tx from one address), `low_tx_count` (fewer than 20 tx observed), `narrow_function_diversity` (at most 3 distinct methods called across the whole range — independent from tx count, since many calls to one function is an equally degenerate sample for a different reason), and `possible_bootstrap_window` (the range starts within ~5000 blocks of the contract's own creation block, via `SourceResolver.get_contract_creation()` — another previously-implemented-but-never-called API wrapper). None of these block the run; they exist purely to make an unrepresentative sample impossible to mistake for a trustworthy one. Any flag firing prints a one-line warning to stderr *before* the expensive trace-enrichment step (so a bad range gets flagged before waiting through a full `--depth deep` run), and shows up as a prominent, un-buried blockquote at the very top of `observed_design.md` — plus the full `sample_quality` breakdown in `observed_design.json`. This is exactly the shape of the Morpho Blue diagnostic run that motivated this whole remediation effort: 12 tx, one caller, 3 admin-only methods, ~324 blocks after deployment — all four flags fire on that range.

**Every run also states plainly whether trace/state-diff evidence was actually obtained, not just how many states came out the other end.** `"No state changes were observed"` used to read identically whether nothing happened on-chain or the RPC provider simply couldn't return traces at all — materially different claims that got conflated. `enrich_transactions()` now tallies real success/failure counts (not just a `structlog` line per failed tx), and `cli.py`'s `compute_trace_coverage()` turns those into an explicit `trace_coverage` block — `"depth_quick"` (skipped this run), `"provider_unsupported"` (attempted for every tx, succeeded for none — a strong signal the configured RPC just doesn't support `debug_traceTransaction`/`trace_transaction`), `"not_attempted"` (nothing to attempt), or no reason at all when real trace evidence was actually obtained. `observed_design.json` carries the full breakdown; `observed_design.md` states the reason in plain language right under the state machine section, and — when the provider genuinely doesn't support tracing — explicitly notes that any transitions shown are grounded in decoded event-log evidence (Fix 1) instead, a materially weaker but still real evidence channel.
...
**Fix**: Not actually an error — OIO continues with calldata/log/receipt-only evidence. Check `observed_design.json`'s `trace_coverage.reason` — `"provider_unsupported"` means this is what happened for this run, and `observed_design.md` states it explicitly under the State Machine section rather than leaving you to infer it from a thin/empty-looking state machine. For full trace/state-diff coverage, use a provider with free tracing (Chainstack, Tenderly) or a local Erigon/Geth node.
